...
class Scan_summary extends BaseController
{
    ...
    public function admin_report_real_time_area()
    {
        // Cek jika user bukan admin, redirect ke halaman lain
        if (session()->get('user_level') !== 'admin') {
            return redirect()->to('/report/user_report');
        ...
        if ($this->request->isAJAX()) {
            ...
            $resumeScan = $scan_model->getScanSummaryAdminFlaviaAreaFl($periode,$startDate,$endDate);
            $resumeScanPerCluster = $scan_model->getScanSummaryAdminFlaviaAreaCluster($periode,$startDate,$endDate);
            $resumeScanPerRegional = $scan_model->getScanSummaryAdminFlaviaAreaRegional($periode,$startDate,$endDate);
            $resumeScanAllRegional = $scan_model->getScanSummaryAdminFlaviaAreaAll($periode,$startDate,$endDate);

            if (empty($resumeScan) || empty($resumeScanPerCluster)) {
                return $this->response->setJSON([
                    'status'  => 'error',
                    'error' => 'No data found',
                    'data'    => []
            ...
            return $this->response->setJSON([
                'maxUpdateDate' => $maxUpdateDate,
                'resumeScan' => $resumeScan,
                'resumeScanPerCluster' => $resumeScanPerCluster,
                'resumeScanPerRegional' => $resumeScanPerRegional,
                'resumeScanAllRegional' => $resumeScanAllRegional
            ]);
        }
        ...
        $data = [
            ...
            'displayInputDate' => $displayInputDate,
            'resumeScan' => $resumeScan,
            'resumeScanRegional' => $resumeScanPerRegional,
            'resumeScanCluster' => $resumeScanPerCluster,
            'resumeScanAllRegional' => $resumeScanAllRegional
        ];

        return view('scan_summary_admin_page_area',$data);
    }
